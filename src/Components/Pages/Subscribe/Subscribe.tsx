  ...
  return (
    <div className='w-full flex flex-col items-center gap-28 '>
        ...
        <div className={checkout? 'os-container rounded-lg  flex flex-col gap-7  items-center' : "hidden"}>
          ...
          <p className='os-p1 text-2xl font-bold text-gray-500 w-72 '>“I drink my coffee as <span className='choice'>{selected1}</span>, with a <span className='choice'>{selected2}</span> type of bean. <span className='choice'>{selected3}</span> ground ala <span className='choice'>{selected4}</span>, sent to me <span className='choice'>{selected5}</span>.”</p>
          <p className='os-p2 leading-6 opacity-80 w-72 text-black'>Is this correct? You can proceed to checkout or go back to plan selection if something is off. Subscription discount codes can also be redeemed at the checkout. </p>
          <button className='btn text-white w-72'onClick={()=> setCheckout(false)} >Checkout -$14.00 / mo</button>
        </div>
    </div>
  )
}
